   ...
   return (
      <div className="block-feature-ten position-relative z-2 pt-150 lg-pt-80">
         <div className="container">
            ...
            <p className="fs-24 text-center font-manrope pb-70 lg-pb-40">Piku for every business you need. we’r provide best offer with best services</p>

            <div className="row justify-content-between">
               {feature_data.map((item) => (
                  <div key={item.id} className="col-xxl-3 col-lg-4">
                     <div className="card-style-five text-center mb-25">
                        <div className="icon d-flex align-items-center justify-content-center rounded-circle"><Image src={icon} alt="Decorative graphic" /></div>
                        <span>{item.title}</span>
                        <p className="font-manrope fw-600 lh-base fs-24 text-white">{item.desc}</p>
                     </div>
                  </div>
               ))}
            </div>
