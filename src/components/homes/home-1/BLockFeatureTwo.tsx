         ...
         <div className="row gx-xl-4">
            {feature_data.map((item) => (
               <div key={item.id} className="col-lg-4 d-flex mt-30 lg-mt-20">
                  <div className="card-style-one text-center border-30 bg-one w-100">
                     <div className="icon rounded-circle d-flex align-items-center justify-content-center m-auto"><Image src={item.icon} alt="Decorative graphic" /></div>
                     <h3>{item.title}</h3>
                     <p className="fs-24 fw-500">{item.desc}</p>
                  </div>
               </div>
            ))}
         </div>
