...
const Counter = () => {
   return (
      <div className="counter-section-one border-top border-bottom border-white border-2 mt-130 lg-mt-80 sm-pt-20 sm-pb-20">
               ...
               {counter_data.map((item) => (
                  <div key={item.id} className={item.class}>
                     <div className={item.class_2 ? item.class_2 : "counter-block d-inline-block"}>
                        ...
                        <p className="text-center fs-22">{item.title}</p>
                     </div>
                  </div>
               ))}
            </div>
