               ...
               {counter_data.map((item) => (
                  <div key={item.id} className="col-lg-4">
                     <div key={item.id} className={`card-style-three position-relative z-1 ${item.class}`}>
                        <Image src={item.logo} alt={`${item.counter_title} icon`} className="logo" />
                        ...
                        <Image src={item.shape} alt="Decorative metric background" className="shapes shape_01" />
                     </div>
                  </div>
               ))}
            </div>
         </div>
         <Image src={counter_shape} alt="Gradient counter backdrop" className="shapes bg-shape-two" />
      </div>
   )
}
