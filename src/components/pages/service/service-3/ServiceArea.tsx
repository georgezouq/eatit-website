          ...
          {feature_data.map((item) => (
            <div key={item.id} className="col-lg-4">
              <div className="card-style-fourteen mt-35">
                <Image src={item.icon} alt="Decorative graphic" className="m-auto icon" />
                <h4>{item.title}</h4>
                <Link href="/service-details" className="cta-btn border-2 border-top border-start border-white tran3s d-flex align-items-center justify-content-center">
                  <Image src={btn_icon} alt="Decorative graphic" /></Link>
              </div>
            </div>
          ))}
        </div>
